...
public class NewCardiacClViewController {

    @FXML
    private AnchorPane cardiacPane;
...
//    ClinicDAO clinicDAO = new ClinicDAOImpl();
    ...
    void backOnClick(ActionEvent event) throws IOException {
        cardiacPane.getChildren().clear();
        AnchorPane pane = FXMLLoader.load(getClass().getResource("/view/ClinicDocView.fxml"));
        cardiacPane.getChildren().add(pane);
    }

    @FXML
    void cardiacPSearch(ActionEvent event) throws SQLException {
//        ClinicModel clinicModel = new ClinicModel();

        if (txtPatientId.getText().isEmpty()) {
            Alert alert = new Alert(Alert.AlertType.WARNING);
            alert.setTitle("Warning");
            alert.setHeaderText("Fill out the Patient Id first!");
            alert.showAndWait();
        } else {
            PatientDetailsDTO patientDetailsDTOS = clinicBO.searchPatientDetails(txtPatientId.getText());
            String name = patientDetailsDTOS.getName();
            lblName.setText(name);
            String bloodG = patientDetailsDTOS.getBloodGroup();
            lblBloodG.setText(bloodG);
        }
    }
        @FXML
        void updateOnCick(ActionEvent event) throws SQLException {
            PatientClinicDetailDTO patientClinicDetailDTO = new PatientClinicDetailDTO();
            if(txtPatientId.getText().isEmpty() || txtPlan.getText().isEmpty() || txtDate.getValue() == null|| txtMedi.getText().isEmpty() || txtSymp.getText().isEmpty() || txtDiagnosis.getText().isEmpty()){
                Alert alert = new Alert(Alert.AlertType.WARNING);
                alert.setTitle("Warning");
                alert.setHeaderText("Fill all the fields first!");
                alert.showAndWait();
            }else {
                patientClinicDetailDTO.setPatientId(txtPatientId.getText());
...
//                ClinicModel clinicModel = new ClinicModel();
    ...
    void showList(ActionEvent event) throws IOException {
        cardiacPane.getChildren().clear();
        AnchorPane pane = FXMLLoader.load(getClass().getResource("/view/CardiacListView.fxml"));
        cardiacPane.getChildren().add(pane);
    }
...
//        ReportDAO reportDAO = new ReportDAOImpl();
        ...
        ReportDTO object = reportBO.showReport(txtPatientId.getText());
